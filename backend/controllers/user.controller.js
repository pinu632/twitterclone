...
export const followUnfollowUser = async (req,res)=>{
    try {
        ...
        if(id === req.user._id.toString()){
            return res.status(400).json({error:"you cant follow and unfollow yourself"});
        }

        if(!userToModify || !currentUser) return res.status(400).json({error:"User not found"});

         const isFollowing = currentUser.following.includes(id); //check if user present in array or not
         ...
         if(isFollowing){
            ...
            const newNotofication = new Notification({
                type:"follow",
                from:req.user._id,
                to:userToModify._id,
            })
            await newNotofication.save();
